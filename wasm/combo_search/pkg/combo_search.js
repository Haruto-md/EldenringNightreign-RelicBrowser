/* @ts-self-types="./combo_search.d.ts" */
 ...
 * @returns {number}
 */
export function progress_counter_ptr() {
    const ret = wasm.progress_counter_ptr();
    return ret >>> 0;
}
 ...
 * @returns {any}
 */
export function search_combinations(input) {
    const ret = wasm.search_combinations(input);
    return ret;
}
...
async function __wbg_init(module_or_path, memory) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('combo_search_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports(memory);
